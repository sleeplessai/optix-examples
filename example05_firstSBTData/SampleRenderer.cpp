...
namespace osc {
  ...
  void SampleRenderer::buildSBT()
  {
    ...
    for (int i=0;i<numObjects;i++) {
      ...
      rec.data.vertex.data = (float3*)vertexBuffer.d_pointer();
      rec.data.vertex.size = vertexBuffer.sizeInBytes / sizeof(float3);
      rec.data.index.data = (int3*)indexBuffer.d_pointer();
      rec.data.index.size = indexBuffer.sizeInBytes / sizeof(float3);
      rec.data.color = *((float3*)(&model.color));
